            ...
            <div class="col-lg-4">
                <x-card>
                    ...
                    <div class="donasi mt-2 mb-4">
                        <a href="{{ url('/donation/'. $campaign->id .'/create') }}"
                            class="btn btn-primary btn-lg btn-block">Donasi Sekarang</a>
                    </div>

                    <h4 class="font-weight-bold">Donatur
                        ({{ $campaign->donations->where('status', 'confirmed')->count() }})</h4>
                    ...
                    <div class="tab-content" id="pills-tabContent">
                        <div class="tab-pane fade show active" id="pills-waktu" role="tabpanel"
                            ...
                            @forelse ($campaign->donations->where('status',
                            'confirmed')->sortBy('created_at')->load('user') as $key => $item)
                        ...
                        <div class="tab-pane fade" id="pills-jumlah" role="tabpanel" aria-labelledby="pills-jumlah-tab">
                            @forelse ($campaign->donations->where('status',
                            'confirmed')->sortBy('nominal')->load('user') as $key => $item)
                            <div @if ($key> 0) class="mt-1" @endif>
                                @if ($item->anonim)
                                <p class="font-weight-bold mb-0">{{ sembunyikan_text($item->user->name, 3) }}</p>
                                @else
